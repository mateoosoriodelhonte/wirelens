...
`diagnostics`. Phase 1 uses classic PCAP timing. It enforces a 64 MiB input cap
and a 65,536-packet cap. Packet and flow IDs are deterministic:
`packet-<one-based-number>` and `tcp-flow-<one-based-number>`. Protocol names
are uppercase, such as `ETHERNET`, `IPV4`, and `TCP`.
...
precision in JavaScript. Byte counts and offsets are non-negative integers. The
packet cap also keeps the maximum cumulative original-byte count below the
...
packet-count limit failures, truncated headers or data, unsupported magic or
version, unsupported link type, invalid packet lengths, and invalid timestamp
conversion. An error may include a capture offset and packet number. No
exception crosses these boundaries.
