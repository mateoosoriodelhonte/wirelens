...
const fakeModule = (overrides: Partial<WasmModule> = {}): WasmModule => {
  ...
  return {
    ...
    wirelens_result_error_packet_number: () => Number.MAX_SAFE_INTEGER,
...
const pcapWithLargeOriginalByteTotal = (): ArrayBuffer => {
  ...
  for (const offset of [24, 40]) {
    view.setUint32(offset + 8, 0, true);
    view.setUint32(offset + 12, 3_000_000_000, true);
  }
  return bytes.buffer;
};

const pcapAbovePacketLimit = (): ArrayBuffer => {
  const packetCount = 65_537;
  const bytes = new Uint8Array(24 + packetCount * 16);
  const view = new DataView(bytes.buffer);
  bytes.set([0xd4, 0xc3, 0xb2, 0xa1]);
  view.setUint16(4, 2, true);
  view.setUint16(6, 4, true);
...
describe('capture worker with the generated Emscripten module', () => {
  ...
  it('preserves large cumulative original-byte metadata through wasm32', async () => {
    const modulePath = resolve(process.cwd(), 'static/wasm/wirelens.js');
    const wasmPath = resolve(process.cwd(), 'static/wasm/wirelens.wasm');
    expect(existsSync(modulePath) && existsSync(wasmPath)).toBe(true);
    const harness = new WorkerHarness();
    ...
    harness.send({
      type: 'parse',
      requestId: 'request-large-original-total',
      fileName: 'large-original-total.pcap',
      buffer: pcapWithLargeOriginalByteTotal(),
    });
    const complete = await waitForResponse(harness, 'request-large-original-total');
    expect(complete.response).toMatchObject({
      type: 'parse-complete',
      document: {
        capture: { packetCount: 2, originalBytes: 6_000_000_000 },
      },
    });
  });

  it('returns the typed packet limit error from the real wasm parser', async () => {
    const modulePath = resolve(process.cwd(), 'static/wasm/wirelens.js');
    const wasmPath = resolve(process.cwd(), 'static/wasm/wirelens.wasm');
    expect(existsSync(modulePath) && existsSync(wasmPath)).toBe(true);
    const harness = new WorkerHarness();
    ...
    harness.send({
      type: 'parse',
      requestId: 'request-packet-limit',
      fileName: 'too-many-packets.pcap',
      buffer: pcapAbovePacketLimit(),
    });
    const failed = await waitForResponse(harness, 'request-packet-limit');
    expect(failed.response).toMatchObject({
      type: 'failed',
      error: {
        code: 'PACKET_LIMIT_EXCEEDED',
        message: 'Capture exceeds the 65,536 packet limit',
        packetNumber: 65_537,
      },
    });
  });
  ...
  it('preserves the native invalid-timestamp error code at the worker boundary', async () => {
    const harness = new WorkerHarness();
    const module = fakeModule({
      wirelens_result_error_code: () => 32,
      wirelens_result_error_offset: () => 24,
    });
    module.HEAPU8.set(new TextEncoder().encode('INVALID_TIMESTAMP\0'), 32);
    createCaptureWorker(harness, async () => module);
    ...
    const failed = await waitForResponse(harness, 'request-timestamp');
    expect(failed.response).toMatchObject({
      type: 'failed',
      error: { code: 'INVALID_TIMESTAMP', captureOffset: 24 },
    });
  });

  it('rejects an oversized transferred buffer before WebAssembly allocation', async () => {
    const harness = new WorkerHarness();
    let allocations = 0;
    const module = fakeModule({
      wirelens_alloc: () => {
    ...
    harness.send({
      ...
      fileName: 'oversized.pcap',
      buffer: new ArrayBuffer(MAX_CAPTURE_BYTES + 1),
    });
    const failed = await waitForResponse(harness, 'request-too-large');
    expect(failed.response).toMatchObject({
      type: 'failed',
