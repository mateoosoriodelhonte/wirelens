...
uintptr_t wirelens_alloc(const size_t size) noexcept {
  if (size == 0 || size > wirelens::kMaxCaptureBytes)
    return 0;
...
uint64_t wirelens_result_error_packet_number(const uint32_t handle) noexcept {
  return registry().with(handle, [](const Result* result) -> uint64_t {
    if (result == nullptr)
      return std::numeric_limits<uint64_t>::max();
    const auto* error = std::get_if<wirelens::ParseError>(&result->parsed);
    if (error == nullptr || !error->packetNumber)
      return std::numeric_limits<uint64_t>::max();
    if constexpr (sizeof(std::size_t) > sizeof(uint64_t)) {
      if (*error->packetNumber > std::numeric_limits<uint64_t>::max())
        return std::numeric_limits<uint64_t>::max();
    }
    return static_cast<uint64_t>(*error->packetNumber);
  });
}
