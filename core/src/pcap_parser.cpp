...
bool timestamp(const std::uint32_t seconds, const std::uint32_t fraction, const std::uint64_t multiplier,
               std::string& output) {
  constexpr auto max = std::numeric_limits<std::uint64_t>::max();
  const auto whole = static_cast<std::uint64_t>(seconds) * 1000000000ULL;
  if (fraction != 0 && static_cast<std::uint64_t>(fraction) > (max - whole) / multiplier) return false;
...
ParseResult parse_capture(const std::span<const std::byte> bytes) {
  if (bytes.size() > kMaxCaptureBytes) return error("FILE_TOO_LARGE", "Capture exceeds the 64 MiB limit");
  if (bytes.size() < 24) return error("TRUNCATED_GLOBAL_HEADER", "PCAP global header is truncated");
  const auto magic = std::array<std::uint8_t, 4>{std::to_integer<std::uint8_t>(bytes[0]), std::to_integer<std::uint8_t>(bytes[1]),
                                                  std::to_integer<std::uint8_t>(bytes[2]), std::to_integer<std::uint8_t>(bytes[3])};
  const bool little = magic == std::array<std::uint8_t, 4>{0xd4, 0xc3, 0xb2, 0xa1} || magic == std::array<std::uint8_t, 4>{0x4d, 0x3c, 0xb2, 0xa1};
  const bool big = magic == std::array<std::uint8_t, 4>{0xa1, 0xb2, 0xc3, 0xd4} || magic == std::array<std::uint8_t, 4>{0xa1, 0xb2, 0x3c, 0x4d};
  if (!little && !big) return error("UNSUPPORTED_MAGIC", "Unsupported capture magic", 0);
  const bool nanoseconds = magic == std::array<std::uint8_t, 4>{0x4d, 0x3c, 0xb2, 0xa1} || magic == std::array<std::uint8_t, 4>{0xa1, 0xb2, 0x3c, 0x4d};
  ...
  while (offset < bytes.size()) {
    ...
    if (static_cast<std::size_t>(*captured) > bytes.size() - offset - 16) return error("TRUNCATED_PACKET_DATA", "PCAP packet data is truncated", offset + 16, number);
    std::string time;
    if (!timestamp(*sec, *fraction, nanoseconds ? 1ULL : 1000ULL, time)) return error("INVALID_TIMESTAMP", "Packet timestamp overflows nanoseconds", offset, number);
    ...
    internal::decode_ethernet(bytes.subspan(offset + 16, *captured), offset + 16, packet.packet, packet.tcp);
    packet.packet.summary = packet.tcp.valid ? internal::flag_text(packet.tcp.flags) : (packet.packet.layers.empty() ? "Truncated frame" : "Ethernet frame");
    parsed.push_back(std::move(packet));
    if (!capture.capture.startTimestampNs) capture.capture.startTimestampNs = time;
  ...
  for (auto& packet : parsed) capture.packets.push_back(std::move(packet.packet));
  return capture;
}
}  // namespace wirelens
