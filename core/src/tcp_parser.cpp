...
std::string flag_text(const std::uint8_t flags) {
  if ((flags & 0x12U) == 0x12U) return "SYN + ACK";
  std::string result;
...
std::optional<ProtocolLayer> decode_tcp(const std::span<const std::byte> payload, const std::size_t captureOffset,
  ...
  add_field(layer, "destinationPort", std::to_string(u16be(payload, 2)), captureOffset, packetOffset + 2, 2);
  add_field(layer, "sequenceNumber", std::to_string(u32be(payload, 4)), captureOffset, packetOffset + 4, 4);
  add_field(layer, "acknowledgmentNumber", std::to_string(u32be(payload, 8)), captureOffset, packetOffset + 8, 4);
  ...
  add_field(layer, "window", std::to_string(u16be(payload, 14)), captureOffset, packetOffset + 14, 2);
  facts = {true, {}, {}, u16be(payload, 0), u16be(payload, 2), u32be(payload, 4), u32be(payload, 8), flags};
  return layer;
}
}  // namespace wirelens::internal
