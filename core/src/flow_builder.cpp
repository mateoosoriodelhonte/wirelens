...
void build_flows(CaptureDocument& capture, std::vector<ParsedPacket>& packets) {
  ...
  for (auto& parsed : packets) {
    ...
    auto stateIt = std::find_if(states.begin(), states.end(), [&](const FlowState& state) {
      const auto same = state.first.source == tcp.source &&
                        state.first.destination == tcp.destination &&
                        state.first.sourcePort == tcp.sourcePort &&
                        state.first.destinationPort == tcp.destinationPort;
      const auto reverse = state.first.source == tcp.destination &&
                           state.first.destination == tcp.source &&
                           state.first.sourcePort == tcp.destinationPort &&
                           state.first.destinationPort == tcp.sourcePort;
      return same || reverse;
    });
