...
std::string address(const std::span<const std::byte> bytes) {
  return std::to_string(std::to_integer<unsigned>(bytes[0])) + "." + std::to_string(std::to_integer<unsigned>(bytes[1])) + "." +
         std::to_string(std::to_integer<unsigned>(bytes[2])) + "." + std::to_string(std::to_integer<unsigned>(bytes[3]));
}
}  // namespace

std::optional<ProtocolLayer> decode_ipv4(const std::span<const std::byte> payload, const std::size_t captureOffset,
                                          const std::size_t packetOffset, TcpFacts& facts, Packet& packet) {
  ...
  if (protocol == 6) {
    if (const auto tcp = decode_tcp(payload.subspan(headerLength, boundedLength - headerLength), captureOffset,
